...
impl ContextError {
    fn invalid(path: &Path, message: impl std::fmt::Display) -> Self {
        Self {
...
// This explicit diagnostic owns jj probing. Shared Git discovery also serves
// Trace2 ingestion, and constructing Repository would initialize ai storage.
fn discover(cwd: &Path) -> Result<WorkspaceContext, ContextError> {
