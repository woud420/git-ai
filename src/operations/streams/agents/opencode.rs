...
fn extract_opencode_event_timestamp(event: &serde_json::Value) -> Option<u32> {
    ...
    let millis = message
        ...
        .or_else(|| {
            message.get("time_updated").and_then(|value| {
                value
                    .as_i64()
                    .or_else(|| value.as_u64().map(|number| number as i64))
            })
...
impl Agent for OpenCodeAgent {
    ...
    fn read_incremental(
        ...
        reader::read_batch(path, ts_watermark, session_id, self.batch_size)
    }
    ...
    fn extract_event_timestamp(
        ...
        extract_opencode_event_timestamp(event)
            .unwrap_or_else(|| file_time_fallback(file_meta, is_first_event))
    }
...
mod tests {
    ...
    pub(super) fn create_test_db(path: &std::path::Path, message_count: usize) {
        if path.exists() {
    ...
    fn test_parts_are_batch_loaded_not_per_message() {
        ...
        let parts = reader::read_parts_for_messages_through(&conn, "test-session-123", 0, i64::MAX)
            .unwrap();
    ...
    fn test_extract_event_timestamp_from_time_created() {
        ...
        assert_eq!(
            OpenCodeAgent::new().extract_event_timestamp(
                &event,
                &metadata.as_file().metadata().unwrap(),
                true,
            ),
            1706459830
        );
    ...
    #[test]
    fn test_extract_event_timestamp_missing_uses_file_fallback() {
        let event = serde_json::json!({"message": {}});
        let metadata = tempfile::NamedTempFile::new().unwrap();
        let metadata = metadata.as_file().metadata().unwrap();

        assert_eq!(
            OpenCodeAgent::new().extract_event_timestamp(&event, &metadata, false),
            crate::operations::streams::agent::file_time_fallback(&metadata, false)
        );
    }
    ...
    fn test_extract_event_timestamp_does_not_use_file_birthtime() {
        ...
        let metadata = tempfile::NamedTempFile::new().unwrap();
        let metadata = metadata.as_file().metadata().unwrap();
        let file_timestamp = crate::operations::streams::agent::file_time_fallback(&metadata, true);

        let timestamp = OpenCodeAgent::new().extract_event_timestamp(&event, &metadata, true);
        assert_eq!(timestamp, 1706459830);
        assert_ne!(timestamp, file_timestamp);
    }
}
