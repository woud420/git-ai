...
use crate::operations::git::sync_authorship::{NotesExistence, fetch_authorship_notes};
use serde::Serialize;
...
pub fn handle_fetch_notes(args: &[String]) {
    ...
    if crate::config::Config::get().notes_backend_kind() == NotesBackendKind::Http {
        match crate::operations::git::notes_api::warm_cache_for_remote(&repo, &remote_name) {
            Ok(()) => {
