...
impl BashHistoryDatabase {
    pub fn global() -> Result<&'static Mutex<BashHistoryDatabase>, GitAiError> {
        ...
        match db_result {
            ...
            Err(error_msg) => Err(history_error(std::io::ErrorKind::Other, error_msg.clone())),
        }
    }
...
fn history_error(kind: std::io::ErrorKind, message: String) -> GitAiError {
    // Database initialization errors are cached and their Display is persisted.
    PersistenceError::Io {
