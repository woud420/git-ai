...
#[test]
fn restore_discard_explicit_source_ai() {
    check_explicit_discard("mock_ai");
}

#[test]
fn restore_discard_explicit_source_known_human() {
    check_explicit_discard("mock_known_human");
}

#[test]
fn restore_discard_collection_opt_out_preserves_existing_journal() {
...
fn restore_discard_index_only_preserves_worktree_evidence() {
    ...
    .unwrap();
    repo.sync_daemon_force();
    repo.stage_all_and_commit("restaged edit").unwrap();
    repo.filename("tracked.txt")
        .assert_committed_lines(lines!["discarded edit".ai()]);
}

#[test]
fn restore_discard_worktree_only_preserves_staged_evidence() {
    let repo = TestRepo::new();
    let base = prepare_discard(&repo, "mock_ai");
    repo.git(&["add", "tracked.txt"]).unwrap();
    repo.git_without_test_sync_for_test(
...
#[test]
fn restore_discard_preserves_unrelated_pending_file() {
    let repo = TestRepo::new();
    let base = prepare_discard(&repo, "mock_ai");
...
#[test]
fn restore_discard_preserves_later_checkpoint() {
    let temp = tempfile::tempdir().unwrap();
    let gate = temp.path().join("restore-gate");
    fs::write(&gate, "hold").unwrap();
    let spec = format!("restore={}", gate.display());
    let repo =
    ...
    while !gate.with_extension("entered").exists() {
        assert!(
            ...
            "restore did not reach its side-effect gate"
        );
...
#[test]
fn restore_discard_linked_worktree() {
...
#[test]
fn restore_discard_failed_restore_preserves_evidence() {
    let repo = TestRepo::new();
    ...
    repo.sync_daemon_force();
    repo.stage_all_and_commit("after failed restore").unwrap();
    repo.filename("tracked.txt")
        .assert_committed_lines(lines!["discarded edit".ai()]);
}

#[test]
fn restore_discard_sparse_skip_remains_native_failure() {
    let repo = TestRepo::new();
    let base = prepare_discard(&repo, "mock_ai");
    ...
    repo.git_og(&["update-index", "--no-skip-worktree", "tracked.txt"])
        .unwrap();
    repo.stage_all_and_commit("after skipped restore").unwrap();
    repo.filename("tracked.txt")
        .assert_committed_lines(lines!["discarded edit".ai()]);
}
...
#[test]
fn restore_discard_invocation_from_root() {
    check_invocation("root", false);
}
#[test]
fn restore_discard_invocation_from_subdir() {
    check_invocation("subdir", false);
}
#[test]
fn restore_discard_invocation_with_c_flag() {
    check_invocation("-C", false);
}
#[test]
fn restore_discard_invocation_from_root_in_worktree() {
    check_invocation("root", true);
}
#[test]
fn restore_discard_invocation_from_subdir_in_worktree() {
    check_invocation("subdir", true);
}
#[test]
fn restore_discard_invocation_with_c_flag_in_worktree() {
...
fn check_backend(kind: git_ai::config::NotesBackendKind) {
    ...
    let repo = TestRepo::new_with_daemon_env_and_patch(
        &[("GIT_AI_API_KEY", "restore-backend-test-key")],
        |patch| {
...
#[test]
fn restore_discard_sqlite_backend() {
    check_backend(git_ai::config::NotesBackendKind::Sqlite);
}
#[test]
fn restore_discard_http_backend() {
    check_backend(git_ai::config::NotesBackendKind::Http);
}

#[test]
fn restore_discard_uses_own_worktree_head_after_another_worktree_commit() {
    let repo = TestRepo::new();
    let base = prepare_discard(&repo, "mock_ai");
...
#[test]
fn restore_discard_git_process_count_does_not_grow_with_pending_files() {
    let temp = tempfile::tempdir().unwrap();
    ...
    println!("restore daemon Git process counts for 1 and 8 pending files: {counts:?}");
}

#[test]
fn restore_discard_directory_and_config_override_keep_existing_boundary() {
...
#[test]
fn restore_discard_plain_path_with_proven_absolute_root() {
    let repo = TestRepo::new();
    let base = prepare_discard(&repo, "mock_ai");
    let root = repo.git_og(&["rev-parse", "--show-toplevel"]).unwrap();
    repo.git_without_test_sync_for_test(
