use crate::config;
use crate::operations::commands::git_hook_handlers::ENV_SKIP_MANAGED_HOOKS;
...
pub fn handle_git(args: &[String]) {
    ...
    let is_read_only = parsed.command.as_deref().is_some_and(|cmd| {
        crate::operations::git::command_classification::is_definitely_read_only_git_invocation(
            cmd,
            &parsed.command_args,
        )
    });
...
fn maybe_show_async_post_commit_stats(parsed: &ParsedGitInvocation, repo: &Repository) {
    ...
    use crate::operations::authorship::stats::{stats_for_commit_stats, write_stats_to_terminal};
    ...
    if let Ok(estimate) = crate::operations::authorship::post_commit::estimate_stats_cost_for_head(
        repo,
        &commit_sha,
        &ignore_patterns,
