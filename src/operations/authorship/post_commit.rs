...
pub use amend::post_commit_amend;
#[allow(unused_imports)]
pub(crate) use amend::{
    PostCommitAmendResult, post_commit_amend_with_recovery_timestamps,
    post_commit_amend_with_recovery_timestamps_detailed,
};
use committed_hunks::recovery_committed_hunks;
#[cfg(test)]
...
use stats_cost::estimate_stats_cost;
pub use stats_cost::{
    STATS_SKIP_MAX_ADDED_LINES, STATS_SKIP_MAX_DELETED_LINES, STATS_SKIP_MAX_FILES_WITH_ADDITIONS,
    STATS_SKIP_MAX_HUNKS, StatsCostEstimate, StatsSkipEstimate, count_line_ranges,
    estimate_stats_cost_for_commit_range, estimate_stats_cost_for_head,
    ...
    F: FnOnce(AuthorshipLog) -> Result<AuthorshipLog, GitAiError>,
{
    ...
    if options.compute_stats {
        ...
        if skip_reason.is_none() {
            let diff_hunks = crate::operations::commands::diff::get_diff_with_line_numbers(
                repo,
                &stats_diff_base,
                &commit_sha,
            )?;
            ...
            let hunks_json = crate::operations::commands::diff::build_diff_artifacts_from_hunks(
                repo,
                diff_hunks,
                &commit_sha,
                Some(&authorship_log),
