...
#[test]
fn orphan_checkout_records_unborn_worktree_without_old_head_fallback() {
    let mut state = family_state();
    state.refs.insert("HEAD".into(), "a".repeat(40));
    state.worktrees.insert(
        PathBuf::from("/tmp/repo"),
        WorktreeState {
    ...
    cmd.invoked_command = Some("checkout".into());
    cmd.invoked_args = vec!["--orphan".into(), "new-root".into()];
    cmd.raw_argv = vec![
        "git".into(),
        "checkout".into(),
        "--orphan".into(),
    ...
    cmd.ref_changes.clear();
    let (_, analysis) = reduce_family_command(&mut state, cmd, &AnalyzerRegistry::new()).unwrap();
    assert_eq!(
        analysis.events,
        vec![crate::model::domain::SemanticEvent::OrphanBranchCreated {
